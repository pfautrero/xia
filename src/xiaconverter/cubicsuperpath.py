...
class cubicsuperpath:
    ...
    def ArcToPath(p1,params):
        ...
        if rx==0 or ry==0:
            return([[A,A,A],[B,B,B]])
        mat=cubicsuperpath.matprod((cubicsuperpath.rotmat(teta),[[1/rx,0],[0,1/ry]],cubicsuperpath.rotmat(-teta)))
        cubicsuperpath.applymat(mat, A)
        cubicsuperpath.applymat(mat, B)
        k=[-(B[1]-A[1]),B[0]-A[0]]
        d=k[0]*k[0]+k[1]*k[1]
        k[0]/=sqrt(d)
        k[1]/=sqrt(d)
        ...
        OA=[A[0]-O[0],A[1]-O[1]]
        OB=[B[0]-O[0],B[1]-O[1]]
        start=acos(OA[0]/cubicsuperpath.norm(OA))
        if OA[1]<0:
            start*=-1
        end=acos(OB[0]/cubicsuperpath.norm(OB))
        if OB[1]<0:
            end*=-1
        ...
        mat=cubicsuperpath.matprod((cubicsuperpath.rotmat(teta),[[rx,0],[0,ry]],cubicsuperpath.rotmat(-teta)))
        for pts in p:
            cubicsuperpath.applymat(mat, pts[0])
            cubicsuperpath.applymat(mat, pts[1])
            cubicsuperpath.applymat(mat, pts[2])
        return(p)
    ...
    def parsePath(d):
        return cubicsuperpath.CubicSuperPath(simplepath.parsePath(d))

    def formatPath(p):
        return simplepath.formatPath(cubicsuperpath.unCubicSuperPath(p))
