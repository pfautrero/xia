...
l'objet}, dans les champs \softmenu{Titre} et \softmenu{Description};
...
\texttt{<message></message>} dans le champ \softmenu{Description} des
\softmenu{Propriétés de l'objet} de l'image de fond\footnote{En créant une
...
Choisir le modèle d'export \softmenu{game1clic} pour générer l'image
interactive.
...
dépôt en faisant correspondre le champ \softmenu{ID} de la zone de dépôt au
champ \softmenu{Description} de l'élément à glisser déposer. La seule
subtilité tient dans la balise \verb|<target></target>| devant être indiquée
dans la \softmenu{Description}.

Choisissez le modèle \softmenu{gameDragAndDrop} pour générer l'image
interactive.
...
infobulle une \softmenu{ID} spéicifique dans les \softmenu{Propriétés de
...
Une autre méthode consiste à insérer un lien dans le champ \softmenu{Titre}
des \softmenu{Propriétés de l'objet} de la zone de dépôt. L'utilisateur peut
