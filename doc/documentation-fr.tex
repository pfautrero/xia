...
Xia n'est nécessaire qu'à la toute fin du processus de création. Comment
vous pouvez le constater sur l'illustration \ref{workflowxia}, l'essentiel
du travail est réalisé avec un logiciel de dessin vectoriel. Nous
recommandons l'utilisation du logiciel libre et multi-plateforme
\href{http://www.inkscape.org/}{Inkscape}, intuitif et simple (Inkscape sera
d'ailleurs utilisé dans cette documentation)\footnote{Il est toutefois
possible d'utiliser LibreOffice Draw.}.
...
Les informations renseignées dans la fenêtre de dialogue des
\chemin{Métadonnées du document} (menu \chemin{Fichier}) seront incluses
dans l'image interactive: le titre, le créateur, les droits, \ldots. Il est
donc fortement recommandé de renseigner ces informations (vous pouvez voir
le résultat final sur l'illustration \ref{titre_ia})\footnote{Les champs
«~auteur~» et «~droits~» apparaissent pour leur part dans la fenêtre «~À
propos~», symbolisé par l'icône cliquable en forme de lettre «~i~».}.
...
ressources, et Xia créera automatiquement un lecteur multimédia, pour peu
...
\begin{description}
 ...
 il suffit alors d'écrire cette url dans le champs \chemin{Description} des
...
Sélectionner des éléments dans l'image. \\ Explications dans la rubrique
...
Glisser-déposer des éléments sur l'image de fond. \\ Explications dans la
...
détails. Pour cela, créez l'infobulle avec un png importé ou une copie
