...
\chemin{Select and transform object} to resize it, to move it\ldots
...
\begin{figure}[htp]
 ...
 \caption{The "\chemin{Object properties}" allows to enter the text that will become a  comment for the detail in the image-active}
...
Clicking on one of the template icons generates a series of files and folders, including a 
\verb|index.html|. 

\begin{alerte}
This file will not display anything if used 
alone. All the other files and directories generated during the export process 
must be stored in  the same folder\ref{xia_files} as the \verb|index.html| file in order to 
the animation to work properly. \textbf{It is therefore essential to 
dedicate a specific directory\ref{xia_files} for each exported image}. Finally, just open 
...
\begin{tikzpicture}[%
  ...
  \node[manualT] (textM) at (10,-2) {These files and folders  were manually created by the image-active designer. The \textcolor{bluedane}
...
in the comments of the image-active using relative links.}; 
  \node[autoT] (textA) at (10,-6) {Files et folders generated by Xia from the svg source file, including the \textcolor{reddebian}{index.html} file.};
...
\begin{tikzpicture}
  ...
  \node[box,above right = 5mm of aCloud.north east] (aBlackC) {\href{http://geoffrey-gekiere.ac-versailles.fr/xia1/accordionBlack}{accordionBlack}\\ Large comment zone, suitable for insertion of multimedia resources; to be used with vertical images (portrait)};
...
In ordre to insert formatted text, the following tags\ref{xia_text_tags} should be used:
...
\begin{description}
 ...
 Just type that url in the \chemin{Description} field of the \chemin{Object Properties} in 
 Inkscape
 
 \item [Relative link] If the multimedia file is located in the image-active 
 folder\ref{xia_files} or in a folder\ref{xia_files} wihtin this one, just indicate its location, keeping in 
...
Inkscape, select \chemin{File $\rightarrow$ Import} to incorporate your new image.
...
By indicating an URL in the \chemin{Title} of \chemin{Object properties} field, 
