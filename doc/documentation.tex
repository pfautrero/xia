...
\begin{figure}[htp]
 ...
 \tikzstyle{ia} = [draw, text width=.8\textwidth, fill=reddebian, rounded corners, inner ysep=4mm]
  % po4a: environement tikzpicture {} 
 \begin{tikzpicture}
   \node[box] (open) {Openimg an Image in Inkscape};
...
\hline
\href{http://geoffrey-gekiere.ac-versailles.fr/xia1/accordionBlack}{accordionBlack}  & \raisebox{-5ex}{\includegraphics[scale=0.3]{./images/accordionBlack}} & Large comment zone, suitable for insertion of multimedia resources; to be used with vertical images (portrait)\\
...
{Description} field of the metadatas of the document (see section \ref{preparation_svg} and illustration \ref{titre_ia}), and create an instruction «~pop up~» 
...
: exact number of details that have to be selected
\footnote{This number does not have to match the number of details on the image) 
...
the {Edition} menu, then on \chemin{Make a bitmap copy}
...
\end{itemize}
 \item In Xia
 \begin{itemize}
  \item Export the svg source file using the «~jeu-DragAndDrop~» template
 \end{itemize}
\end{enumerate}
