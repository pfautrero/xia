...
function main(myhooks) {
    ...
    that.imageObj.onload = function() {
		...
		var anim = new Kinetic.Animation(function(frame) {
		  ...
		  for (var i in mainScene.shapes) {
			mainScene.shapes[i].group.zoomActive = 1;
			var test = mainScene.shapes[i].kineticElement[0].x();
			//mainScene.shapes[i].kineticElement[0].setX(5 * Math.sin(frame.time * 2 * Math.PI / 2000) + mainScene.shapes[i].kineticElement[0].x());
			if (dist+test > mainScene.width) {
				mainScene.shapes[i].kineticElement[0].setX(0);
			}
			else {
				mainScene.shapes[i].kineticElement[0].setX(dist+test);
			}
			
			
			
			layers[parseInt(i+3)].draw();
		  }
		}, null);
